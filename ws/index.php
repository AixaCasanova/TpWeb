...
$app->post('/Encuesta[/{objeto}]', function ($request, $response, $args) {
    
    $obj=json_decode($args['objeto']);
    $calidadAtencionOld = $obj->calidadAtencionOld;
    $ComoLlego = $obj->ComoLlego;
    $PregAdquirioOld = $obj->PregAdquirioOld;
    $calidadProductoOld = $obj->calidadProductoOld;
    $descripcionClaraNew = $obj->descripcionClaraNew;
    $problemasPRealizarPNew = $obj->problemasPRealizarPNew;
    $tiempoEntregaOld = $obj->tiempoEntregaOld;

    $resp= pedidos::InsertarEcuesta($calidadAtencionOld, $tiempoEntregaOld,$ComoLlego,$PregAdquirioOld,$calidadProductoOld,  $descripcionClaraNew, $problemasPRealizarPNew );

    return json_encode($tiempoEntregaOld);
...
$app->get('/TraerUnPedido[/{objeto}]', function ($request, $response, $args) {
 
    $ped=$args['objeto'];
    $res=pedidos::TraerDetPed($ped);
...
$app->post('/ModifSuc[/{objeto}]', function ($request, $response, $args) {

    $suc = json_decode($args['objeto']);
    $id=$suc->id_sucursal;
    $direccion=$suc->direccion;
    $nombre=$suc->nombre;
    $resp=sucursal::ModificarSucursal($id,$direccion,$nombre);
   return json_encode($resp);
});

$app->post('/ElimSuc[/{objeto}]', function ($request, $response, $args) {

    $suc = json_decode($args['objeto']);
    $id=$suc->id_sucursal;
    $direccion=$suc->direccion;
    $nombre=$suc->nombre;
    $resp=sucursal::EliminarSucursal($id);
   return json_encode($resp);
   // return "hola";
});
...
$app->post('/AltaClientes/{objeto}', function ($request, $response, $args) {
 ...
 $app->post('/AltaOfertas/{objeto}', function ($request, $response, $args) {
    ...
    $dato=sucursal::TraerUnasucursalPorNombre($of->sucursal);
